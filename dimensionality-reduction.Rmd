...
description: "Quantum machine learning algorithms. Lecture notes quantum dimensionality reduction, quantum principal component analysis, quantum linear discriminant analysis, quantum slow feature analysis, quantum feature space."
...
```{corollary, qpca-fitting, name="qPCA: fitting time"}
    ...
    the run-time to extract $V^{(k)} \in \R^{m \times k}$ for corollaries \@ref(cor:qpca-vector), \@ref{cor:qpca:matrix} is $\widetilde{O}\left(\frac{\mu(A)k^2m}{\theta\epsilon\xi^2}\right)$.
```
