...
```{definition, name="Eigenvalues and Eigenvectors"}
\cite[Section 6.1 page 289]{linearalgebragilbert}
...
and its determinant is equal to the product of the eigenvalues \cite[Section 6.1 page 294]{linearalgebragilbert}
...
\cite[Definition 7.1.5 page 108]{scapellato}
...
```{lemma, name="Linear independence of eigenvectors"}
\cite[Lemma 7.2.3 page 112]{scapellato}
...
```{theorem, name="Singular Value Decomposition"} 
...
Note that the matrices $AA^T$ and $A^TA$ are symmetric matrices and so, for the Spectral theorem, they can always be decomposed. Moreover, note that they have positive eigenvalues: being the square roots of real positive eigenvalues, the singular values of a real matrix are always real positive numbers.
...
```{definition, name="Column (Row) Space"} 
 \cite[Definition 8.1 page 192]{schlesinger} \label{Def:column_row_space} Let $A$ be a $\mathbb{R}^{n\times m}$ matrix. The column (row) space of $A$ is the space spanned by the column (row) vectors of $A$. Its dimension is equal to the number of linearly independent columns (rows) of $A$.
```

The number $r$ of singular values and singular vectors of a matrix is its rank.
```{definition, name="Rank of a matrix"} 
 \cite[Definition 8.3, Proposition 8.4 page 193-194]{schlesinger} \label{Def:rank} The rank of a matrix is the number of linearly independent rows/columns of the matrix. If the matrix belongs to the $\mathbb{R}^{n\times m}$ space, the rank is less or equal than $min(n,m)$. A matrix is said to be full rank if its rank is equal to $min(n,m)$.
```
...
To begin with, the SVD representation of a matrix allows us to fastly calculate some matrix norms, like the $l_2$ norm and the Frobenius norm.
```{definition, name="$l_2$ (or Spectral) norm"}
 \cite[Section 11.2 page 518]{linearalgebragilbert} \label{Def:spectral_norm} 
...
```{theorem, name="Best F-Norm Low Rank Approximation"}
...
<!--         \emph{Example:} We use an algorithm that gives the log-determinant (of a  matrix with spectral norm smaller than 1) with relative error $\frac{1}{4}$, so $\log\det(1-\frac{1}{4}) \leq \overline{\log\det} \leq \log\det(1+\frac{1}{4}) \mathbb{R}ightarrow \frac{3}{4}\leq \frac{\overline{\log\det}}{\log\det} \leq \frac{5}{4}$. Now we run it with error $\epsilon'_R =  \frac{\epsilon_{Abs}}{4\overline{\log\det}}$.  -->
...
<!-- -  $A=\lambda B \mathbb{R}ightarrow \Delta A = |\lambda | \Delta B$ -->
<!-- -  $A = \lambda/B \mathbb{R}ightarrow  \Delta A =  |\lambda /B^2| \Delta B = (substitute) = |A/B|\Delta B $  -->
<!-- -  $A = \lambda B^\mu \mathbb{R}ightarrow \Delta A = |\mu \lambda B^{\mu^{-1}}|\Delta B = (substitute)= |\mu A /B | \Delta B$  -->
<!-- -  $A = \lambda e^{\mu B} \mathbb{R}ightarrow \Delta A = |\mu A| \Delta B$ (where is $\lambda$?)  -->
<!--     This because if  -->
<!--     $$A = \lambda e^{\mu B}$$ -->
...
<!-- -  $A = \kambda \ln (\mu B) \mathbb{R}ightarrow \Delta A = |\lambda/B|\Delta B$  -->
...
<!-- $$|AB- (A+\epsilon_1)(B+\epsilon_2)| = | AB - AB-A\epsilon_2 -B\epsilon_1 - \epsilon_1\epsilon_2| = |-A\epsilon_2 - B\epsilon_1 + O(\epsilon_1 \epsilon_2) | \mathbb{R}ightarrow$$ -->
