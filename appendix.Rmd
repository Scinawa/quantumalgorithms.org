...
The triangle inequality is basically the [subadditivity](https://en.wikipedia.org/wiki/Subadditivity) property of the norm. 
...
$$(a+b)^n = \sum_{k=0}^n \choose{n, k}a^kb^{n-k}$$
```
...
Note that we could have also bypassed the union bound and compute this quantity analitically, as the probability of getting 4 success in a row would be $(1-\delta)^4$, which we can compute with the \@ref(binomial-theorem). 
```
...
Let $p,q>1$ that satisfy $\frac{1}{p} + \fac{1}{q} = 1$. If $\|X\|_p \leq \infty$ and $\|X\|_q$ then
