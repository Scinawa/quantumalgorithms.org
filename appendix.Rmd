...
```{proof}
Observe that $(X-\mu)_2$ is a non-negative random variable. Therefore,
$$P(|X-\mu| \geq \epsilon) = P((X-\mu)^2 \geq \epsilon^2)$$
Now since $(X-\mu)_2$ is a non-negative random variable, we can apply Markov inequality to get :
...
```{proof}
We know that $E[\bar{X}] = \mu$ and $Var(\bar{X}) = \frac{\sigma^2}{n}$. By Chebyshev Inequality,
$$P(|\bar{X}-\mu| > \epsilon) \leq \frac{Var(\bar{X})}{\epsilon^2} = \frac{\sigma^2}{n\epsilon^2} \rightarrow 0 (as n \rightarrow \infty)$$
```

### (Strong) Law of Large Numbers

```{theorem, slln, name="(Strong) Law of large numbers"}
Let $X_1,X_2,X_3,...,\X_n$ be i.i.d random variables with mean $\mu$. Let $\bar{X} = \sum_{i=1}^{n} X_i$ be the sample mean. Then, $\bar{X}$ converges almost surely to $\mu$.That is,
