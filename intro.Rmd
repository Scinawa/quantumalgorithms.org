...
Following one of the [lectures of Sussking](https://www.youtube.com/watch?v=8mi0PoPvLvs), we are going to start from a "handwavy" introduction that justify the existence of the Schrödinger equation. With a few mathematical observation we are going to give an intuitive justification of all the 4 axioms of quantum mechanics that are stated in the next sections. The hope is that, thanks to this introduction, the reader can be gently guided from a tiny physical intuition, to a greater understanding of the axioms of quantum mechanics. Despite their name might seems too much related to physics, thanks to this formulation (which comes from [@NC02]), we could eventually call them "axioms of quantum computing". 
...
When, at the beginning of the 20th century, physicist started to model quantum phenomena, they observed that the dynamic of the systems had two properties. The time and space evolution of quantum systems were continious (as in classical mechanics) and reversible (unlike the classical world). They decided to formalize this concept as follow. First they decided to model the state of a quantum system at time $p$ as a function $\psi(p)$, and they decided to model the evolution of $\psi(p)$ for time $t$ as an operator $U(t)$ acting on $\psi(p)$s. Formally, the two requirements can be written as:

- $U(\epsilon)=I-i\epsilon H$ (continuity)
- $U^\dagger(t)U(t)=I$ (reversibility)

The first requirement reads that if we were to apply out evolution for a small amount of time $\epsilon$, than $U$ would behave almost as the identiy, and then it will apply for a "small" amount another operator $H$. The second requirement reads that if we "undo" the operator $U$, by applying the transpose conjugate, we would obtain the identity, i.e. we haven't change the state of the system. 
...
In mathematics we have a name for such operators, and they are called Hermitian operators! (More about those in the appendix!).
...
The left hand side (l.h.s) part of this equation can be rewritten as a derivative:
...
$$\psi(p+t)=e^{-iHt}\psi(p)$$
From this last equation we can observe further (more on this in the appendix) that the exponential of an Hermitian matrix $e^{-iHt}$ is *defined* through it's Taylor expansion is just a *unitary* matrix: $U(t)=e^{-itH}$. Unitary matrices are exactly those matrices that describe isometries: applying a matrix to a vector won't change its lenght. From this we see that the two quantum state $\psi(p+t)$ and $\psi(p)$ could be just taken to be vector of a fixed length, which - for practicality - we take to be unit vectors. Notation-wise, we denote unit vectors describing quantum states as "kets", i.e. we rewrite this quation as:
...
## Loading the data: memory models
...
```{definition, oracle-access-adjacencylist,  name="Oracle access in adjacency list model"}
	Let $V \in \mathbb{R}^{n \times d}$, there is an oracle that allows to perform:

- the mapping $\ket{i}\mapsto\ket{i}\ket{d(i)}$ where $d(i)$ is the number of entries in row $i$, and
- $\ket{j,l}\mapsto\ket{j,\nu(j,l)}$, where $\nu(j,l)$ is the $l$-th nonzero entry of the $j$-th column of $V$.
