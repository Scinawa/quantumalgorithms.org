...
description: "The best up-to-date resource for algorithms and applications. From Theorems to applications. Deutsch-Josza, Bernstein-Vazirani, quantum swap test, quantum Hadamard test. Representing data on quantum computers. Retrieveing data on quantum computers"
---
...
It will be useful to recall that if we have a unitary that performs the mapping $\ket{a_i} \mapsto \ket{b_i}$, we can have the "matrix" form of the operator as $\sum_i \ket{b_i}\bra{a_i}$.
...
They have some useful properties. Just to cite one, the average value of a projective measurement in a state $\ket{\psi}$ is defined as:
\begin{align}
	E(M)&  = \sum_m p(m)\\
...
## Representing data in quantum computers {#representing-data}
...
We start by focusing on representing numbers. In this context, we want to build a unitary operator $U$ that performs the following mapping:
$$U\ket{i}\ket{0}\to \ket{i}\ket{x_i}$$
Where $\ket{x_i}$ is the $i$-th element of our input, which can be a vector or a number, as we will see now. As a convention, throughout this book, we often use Greek letters inside kets to represent generically quantum states, and use Latin letters to represent quantum registers holding classical data.
...
####  Vectors and matrices: $\mathbb{R}^n$, $\mathbb{R}^{n \times d}$ {#subsec-stateprep-matrices}
...
#### Quantum arithmetic models

We assume that we can have enough qubits for storing these numbers, represented as bit strings using the following definition. We also assume to work with enough precision so that numerical errors in the computation are negligible, and will not impact the final output of our algorithm. 
...
It might seem complicated, but it's really the (almost) only thing that a reasonable person might come up with when expressing numbers as (qu)bits with fixed-point precision. 
...
<!-- In our computational model we do not include the cost for performing operations described in our arithmetic model. For instance, a central computational step of the quantum algorithm is the circuit computing the stopping times $\widetilde{\tau}_t(x)$, but as the circuit depth depends polynomially on $c_1$ and $c_2$, we do not take into account this cost when stating our runtime. -->
<!-- For an example of a resource estimation for a financial problem that takes into account the cost of arithmetic operations in fixed-point precision, we refer to~\cite{chakrabarti2021threshold}. -->




## Loading data: quantum memory models {#loading-data}
...
Remember: we work in a **query model** of quantum computation, that we defined in Section \@ref(measuring-complexity). This section is devoted to the formalization and implementation of some of the oracles that are commonly assumed in literature, and thus in this book. 

### The QRAM model {#subsec:qram-model}
...
### Quantum sampling access and quantum access to a matrix
