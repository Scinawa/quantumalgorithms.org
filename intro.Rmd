...
In this chapter we will introduce the notation that we will extensively use. In this lecture notes will use extensively linear algebra (norm of matrices, SVD, properties of particular matrices, and so on). The reader is higly encouraged to skim the appendix (at least the first part on linear algebra) to have a knowledge of the notation used here. 
...
### Measuring complexity quantum algorithms
...
Equipped with this data structure - which we have not discussed in depth yet, but it's discussed here [@giovannetti2008quantum] - allows us to do the following mappings. 

<!-- TODO
add here a little discussion on hardware and circuit for QRAM
-->

```{definition, oracle-access-vector, name="Query access to a vector"}
...
```{definition, oracle-access-adjacencymatrix, name="Oracle access in adjacency matrix model"}
...
Note that the first is very similar to the definition of the Grover's oracle that you might know from previous courses in quantum information. It's important to remember that for Definition \ref(def:oracle-access-adjacencymatrix) and \ref{def:oracle-access-adjacencylist} we might **not** need a QRAM, as there might be other efficient circuit for performing those mapping. This is usually the case in all the literature for quantum algorithms for Hamiltonian simulation, that we will see better in Chapter \ref{chap:toolbox}.  These query models are sometimes called in literature "sparse access" to a matrix, as sparsity is often the key to obtain an efficient circuit for encoding such structures without a QRAM. 
